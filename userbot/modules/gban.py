...
async def gben(userbot):
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit(
                f"**Lo ga bisa banned philaxx, dia devloper gue goblok!!!**"
            )
        try:
    ...
    return await dark.edit(
        ...
        f"││➩ **Aksi:** `Global Banned` \n"
        f"╰┈─────────────━\n\n"
        f"❂ __Gbanned by ☠𝕻𝖍𝖎𝖑𝖆𝖝𝖝-𝖀𝖇𝖔𝖙☠__")
...
async def gunben(userbot):
    ...
    if user:
        if user.id in DEVS:
            return await dark.edit("**LU GABISA BLACKLIST PHILAXX, DIA YG BIKIN GUA NGENTOT!!!**")
        try:
